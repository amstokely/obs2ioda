      ...
      if ( .not. fexist ) then
         ...
         if ( nfgat > 1 ) then
            do itime = 1, nfgat
               ...
               call write_obs(filedate_out, write_nc_conv, outdir, itime, ncfname)
               print *, trim(ncfname)
            end do
         else
            call write_obs(filedate, write_nc_conv, outdir, 1, ncfname)
            print *, trim(ncfname)
         end if
...
if ( do_radiance ) then
   ...
   if ( nfgat > 1 ) then
      do itime = 1, nfgat
         ...
         call da_advance_time(filedate, trim(dtime), datetmp)
         filedate_out = datetmp(1:10)
         call write_obs(filedate_out, write_nc_radiance, outdir, itime, ncfname)
         print *, trim(ncfname)
      end do
   else
      call write_obs(filedate, write_nc_radiance, outdir, 1, ncfname)
      print *, trim(ncfname)
   end if
   if ( allocated(xdata) ) deallocate(xdata)
end if
...
if ( do_ahi ) then
   ...
   call write_obs(filedate, write_nc_radiance_geo, outdir, 1, ncfname)
   print *, trim(ncfname)
   if ( allocated(xdata) ) deallocate(xdata)
end if
